...
def op_param_simulation_phase(v:np.array):
    ...
    # val = simdata.qpos[-2] - np.abs(simdata.qpos[-1])
    print(f"x축 위치 : {simdata.qpos[-2]} , y축 위치 : {simdata.qpos[-1]} , 총 변위 : {float(np.sqrt( simdata.qpos[-2] ** 2 + simdata.qpos[-1]) ** 2)}")

# Simulation example code
try:
    in1 = float(input("첫번째 Gait 변수 입력 (Dorsal spatial parameter 0도 ~ 360도) :"))
except:
    print("변수 타입을 확인하세요. 실수입력!")
    exit()
try:
    in2 = float(input("두번째 Gait 변수 입력 (Lateral spatial parameter 0도 ~ 360도) :"))
except:
    print("변수 타입을 확인하세요. 실수입력!")
    exit()
try:
    in3 = float(input("세번째 Gait 변수 입력 (Dorsal temporal parameter 1 ~ 4) :"))
except:
    print("변수 타입을 확인하세요. 실수입력!")
    exit()
try:
    in4 = float(input("네번째 Gait 변수 입력 (Lateral temporal parameter 1 ~ 4) :"))
except:
    print("변수 타입을 확인하세요. 실수입력!")
    exit()
try:
    in5 = float(input("다섯번째 Gait 변수 입력 (Delta degree 0도 ~ 360도) :"))
except:
    print("변수 타입을 확인하세요. 실수입력!")
    exit()
try:
    in6 = float(input("모터의 동작 세기 (0.0 ~ 3.0):"))
except:
    print("변수 타입을 확인하세요. 실수입력!")
    exit()

gait_param = np.array([in1,in2,in3,in4,in5])
M = genMotionMat(serpenoid(t_range, gait_param[0], gait_param[1], gait_param[2], gait_param[3], gait_param[4])).copy()

op_param_simulation_phase(gait_param)
