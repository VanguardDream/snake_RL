...
def orderize_linear(param_motion:np.ndarray, curve:bool, isSlit:bool, param_bar_iter:np.ndarray, shd_name:str, shd_shape, visual:bool, savelog:bool) -> None:
...
def iterator_linear(motion:np.ndarray, curve:bool, visual:bool = False, savelog:bool = False) -> None:
    ...
    pc1 = Process(target= orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[0]:start_idx[1]]),   shm.name, U_map.shape, visual, savelog))
    pc2 = Process(target= orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[1]:start_idx[2]]),   shm.name, U_map.shape, visual, savelog))
    pc3 = Process(target= orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[2]:start_idx[3]]),   shm.name, U_map.shape, visual, savelog))
    pc4 = Process(target= orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[3]:start_idx[4]]),   shm.name, U_map.shape, visual, savelog))
    pc5 = Process(target= orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[4]:start_idx[5]]),   shm.name, U_map.shape, visual, savelog))
    pc6 = Process(target= orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[5]:start_idx[6]]),   shm.name, U_map.shape, visual, savelog))
    pc7 = Process(target= orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[6]:start_idx[7]]),   shm.name, U_map.shape, visual, savelog))
    pc8 = Process(target= orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[7]:start_idx[8]]),   shm.name, U_map.shape, visual, savelog))
    pc9 = Process(target= orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[8]:start_idx[9]]),   shm.name, U_map.shape, visual, savelog))
    pc10 = Process(target=orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[9]:start_idx[10]]),  shm.name, U_map.shape, visual, savelog))
    pc11 = Process(target=orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[10]:start_idx[11]]), shm.name, U_map.shape, visual, savelog))
    pc12 = Process(target=orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[11]:start_idx[12]]), shm.name, U_map.shape, visual, savelog))
    pc13 = Process(target=orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[12]:start_idx[13]]), shm.name, U_map.shape, visual, savelog))
    pc14 = Process(target=orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[13]:start_idx[14]]), shm.name, U_map.shape, visual, savelog))
    pc15 = Process(target=orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[14]:start_idx[15]]), shm.name, U_map.shape, visual, savelog))
    pc16 = Process(target=orderize_linear, args=(np.array(motion_param), curve, isSlit, list(combinations[start_idx[15]::]),             shm.name, U_map.shape, visual, savelog))
    ...
    savemat("./U_map_gamma_chgd_"+str(curve)+"_"+param2filename(motion_param)+f"_{len(combinations)}"+"_.mat", data_dict)
...
if __name__ == "__main__":
    ...
    # print(J(rolling, (45, 45, 0, 0, 59/2, 53/2, 45, 0.05), True, False))
    # exit()
