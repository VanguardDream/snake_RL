...
def J_traj_each(parameters:np.ndarray, parameters_bar:np.ndarray, u_weight:np.ndarray, curve:bool, gamma:float) -> np.ndarray:
    ...
    for i in range(expand_q.shape[1]):
        ...
        # Trajectory Hist
        hist_x = int(data.body('head').xpos[0] * 300 + 250)
        hist_y = int(data.body('head').xpos[1] * 300 + 250)

        if traj[np.clip(hist_x,0,499), np.clip(hist_y,0,499)] == 0:
            traj[np.clip(hist_x,0,499), np.clip(hist_y,0,499)] = 1
    ...
    reshape_traj = np.reshape(traj, (250000,))

    return np.hstack((i_term, j_term, l_term, mean_rot, reshape_traj))
...
def for_iter(lambda_m, u_w, shd_name:str, shd_shape):
    ...
    for i in u_w:
        ...
        tmp_traj = x_fcn[-250000::]
        tmp_traj = np.reshape(tmp_traj, (500,500))

        U_map[int(i[0]),::] = tmp_traj
...
if __name__ == "__main__":
    ...
    U_map = np.zeros((data_size, 500, 500))
