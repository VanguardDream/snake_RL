...
def main(ia, ia2):
    ...
    for i in range(0,183):
        ...
        goal = serpenoid(i, b, b2,a, a2, e_d1, e_d2, e_l1, e_l2, delta)
        ...
        for idx in range(14):
            dxl_addparam_result = gsread_PP.addParam(idx, ADDR_PRESENT_POSITION, LEN_PRESENT_POSITION)
            if dxl_addparam_result != True:
                print("[ID:%03d] groupSyncRead addparam failed" % idx)
                quit()

        for idx in range(14):
            dxl_addparam_result = gsread_PV.addParam(idx, ADDR_PRESENT_VELOCITY, LEN_PRESENT_VELOCITY)
            if dxl_addparam_result != True:
                print("[ID:%03d] groupSyncRead addparam failed" % idx)
                quit()
...
if __name__ == "__main__":
    ...
    gsread_PP = GroupBulkRead(portHandler, packetHandler)
    gsread_PV = GroupBulkRead(portHandler, packetHandler)
