...
class PlaneJoyWorldCG(MujocoEnv, utils.EzPickle):
    metadata = {
        "render_modes": [
            "human",
            "rgb_array",
    ...
    def __init__(
        ...
        _period = int(( 1 / (self.model.opt.timestep * self.frame_skip) ) * 0.15)
        _period2 = int(( 1 / (self.model.opt.timestep * self.frame_skip) ) * 0.15)

        self._mov_mean_vels = MovingAverageFilter3D(window_size=_period)
        self._mov_gait_ypr = MovingAverageFilterQuaternion(window_size=_period)
        ...
        obs_size = self.data.sensordata.size + 3 ## Motion vector + Joy input
    ...
    def step(self, action):
        ...
        info = {
            ...
            "joint_tor": observation[28:42].copy(),
            "head_quat": observation[-13:-9].copy(),
            "head_ang_vel": observation[-9:-6].copy(),
            "head_lin_acc": observation[-6:-3].copy(),
    ...
    def _get_obs(self):
        ...
        return np.concatenate((tmp.flatten(), self._joy_input), dtype=np.float32)
    ...
    def reset_model(self):
        ...
        _period = int(( 1 / (self.model.opt.timestep * self.frame_skip) ) * 0.3)
