...
def J_view(parameters:np.ndarray, parameters_bar:np.ndarray, curve:bool, gamma:float) -> np.ndarray:
    ...
    return : [U, avg. yaw]
    """
    _robot_body_names = ["link1","link2","link3","link4","link5","link6","link7","link8","link9","link10","link11","link12","link13","link14","link15"]
    ...
    snake = mujoco.MjModel.from_xml_path("./resources/horcrux_plane.xml")
    data = mujoco.MjData(snake)
    mujoco.mj_forward(snake, data)
    ...
    with mujoco.viewer.launch_passive(snake, data) as viewer:
        ...
        for i in range(expand_q.shape[1]):
            ...
            step_data = np.hstack((data.body('link1').xpos.copy(), get_robot_rot(data), data.ctrl))
            p_head[i] = step_data
...
def J_traj_each(parameters:np.ndarray, parameters_bar:np.ndarray, curve:bool, gamma:float) -> np.ndarray:
    ...
    for i in range(expand_q.shape[1]):
        ...
        step_data = np.hstack((data.body('link1').xpos.copy(), get_robot_rot(data), data.ctrl, data.sensordata[-3::]))
        p_head[i] = step_data
...
if __name__ == "__main__":
    ...
    # # GD Tor과 비교를 위해서
    tor_linear = (45, 45, 154, 154, 89, 89, 0, 0.05)
    print(J_view(tor_linear,tor_linear,True,0.7071)[0])
