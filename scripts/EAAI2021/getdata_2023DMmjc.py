...
def do_sim(ins_idx:int, start:np.ndarray, end:np.ndarray):
    ...
    internal_result = np.empty((15, 36, 15, 36))
        ...
        for p2 in range(start[1], end[1]):
            ...
            for p3 in range(start[2], end[2]):
                ...
                for p4 in range(start[3], end[3]):
                    ...
                    params = np.array([1, 10 + p1 * 5, p2 * 10, -9.1, 10 + p3 * 5, p4 * 10, 7.0, 1])
...
if __name__ == "__main__":
    ...
    result = J(1, 39.8, 189.9, -9.1, 66.5, 160.9, 7.0, 1)
    print(result)
    # ((41,73,41,73))
    # ((15, 36, 15, 36))

    # th0  = Process(target=do_sim, args=( 0, np.array([ 0, 0, 0, 0]), np.array([ 1, 36, 15, 36]), ))
    # th1  = Process(target=do_sim, args=( 1, np.array([ 1, 0, 0, 0]), np.array([ 2, 36, 15, 36]), ))
    # th2  = Process(target=do_sim, args=( 2, np.array([ 2, 0, 0, 0]), np.array([ 3, 36, 15, 36]), ))
    # th3  = Process(target=do_sim, args=( 3, np.array([ 3, 0, 0, 0]), np.array([ 4, 36, 15, 36]), ))
    # th4  = Process(target=do_sim, args=( 4, np.array([ 4, 0, 0, 0]), np.array([ 5, 36, 15, 36]), ))
    # th5  = Process(target=do_sim, args=( 5, np.array([ 5, 0, 0, 0]), np.array([ 6, 36, 15, 36]), ))
    # th6  = Process(target=do_sim, args=( 6, np.array([ 6, 0, 0, 0]), np.array([ 7, 36, 15, 36]), ))
    # th7  = Process(target=do_sim, args=( 7, np.array([ 7, 0, 0, 0]), np.array([ 8, 36, 15, 36]), ))
    # th8  = Process(target=do_sim, args=( 8, np.array([ 8, 0, 0, 0]), np.array([ 9, 36, 15, 36]), ))
    # th9  = Process(target=do_sim, args=( 9, np.array([ 9, 0, 0, 0]), np.array([10, 36, 15, 36]), ))
    # th10 = Process(target=do_sim, args=(10, np.array([10, 0, 0, 0]), np.array([11, 36, 15, 36]), ))
    # th11 = Process(target=do_sim, args=(11, np.array([11, 0, 0, 0]), np.array([12, 36, 15, 36]), ))
    # th12 = Process(target=do_sim, args=(12, np.array([12, 0, 0, 0]), np.array([13, 36, 15, 36]), ))
    # th13 = Process(target=do_sim, args=(13, np.array([13, 0, 0, 0]), np.array([14, 36, 15, 36]), ))
    # th14 = Process(target=do_sim, args=(14, np.array([14, 0, 0, 0]), np.array([15, 36, 15, 36]), ))
